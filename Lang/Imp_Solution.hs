...
readVariable' n = ImpExpr $ \st -> 
  maybe (Left $ "variable not found: " ++ n ) Right (M.lookup n st)
...
assertNonZero :: Integer -> ImpExpr Integer
assertNonZero n = 
  if n == 0 
...
exec CSkip st = pure st
exec (CAssign n v) st = do
  value <- runExpr (eval v) st
  pure $ M.insert n value st
exec (CIfThEl b t e) st = do
  cond <- runExpr (eval b) st
  exec (if cond then t else e) st
exec (CWhile b c) st = do
  cond <- runExpr (eval b) st
  if cond 
    then do
      st' <- exec c st
